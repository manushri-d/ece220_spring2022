...
/* In this MP, using the data structures modeled in the .h file, I completed the methods.
The is_leaf_node method, is_internal_node just used if statements to check the left and right
nodes. The is_in_subtree recursively called itself for left aand right nodes. The rotate
method used a standard swap method to change the h and w. The recut method used if statements
to change the V and H of the cutline. The swap_module method used a standard swap method to
change a->module and b->module. The swap_topology method used a standard swap method and then
changed the left and right childs using if statements. The postfix_traversal method first had
a recursive call to traverse the left and right trees, then checked if the node was a cutline
or a module, and set the module/cutline to its respective values. The get_total_resource
method used a recursive calling to traverse left and right. The init_slicing_tree method had
temp nodes that were set to each statement for the node struct as outlined in the .h file. I had
a base case, and a recursive case(internal node, right node, left node).*/
...
int is_leaf_node(node_t* ptr) {
  ...
  if (ptr->right == NULL && ptr->left == NULL){ //right and left childs are empty, aka leaf node
    return 1; 
  }else{
    return 0; 
  }
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  if (ptr->right != NULL && ptr->left != NULL){ //right and left childs are not empty, aka internal node
    return 1;
  }else{
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
  if(a != NULL){//would normally be a while loop, but since it is a recursive method, use if statement
    if(a == b || is_in_subtree(a->left, b) || is_in_subtree(a->right, b)){ //checks if the nodes are equal to each other and if node b is in childs of a recursively
      return 1;
    }
  }else{
    return 0; //NULL pointer
...
void rotate(node_t* ptr) {
  ...
  if(ptr != NULL){ //check if pointer is null
    temp = ptr->module->w; //set a temp to the w of the module
    ptr->module->w = ptr->module->h;
    ptr->module->h = temp; //swap method(standard)
  }
}
...
void recut(node_t* ptr) {
  ...
  // TODO:
  if(ptr->cutline == H){ //if it is H
    ptr->cutline = V; //change to V
  }else{
    ptr->cutline = H; //set to H
  }
  return;
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
  module_t * temp = b->module; //set a temp to b's module
  b->module = a->module; 
  a->module = temp; //swap method(standard)
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
  node_t * temp = a->parent; //temp to a's parent
  a->parent = b->parent;
  b->parent = temp; //swap method(standard) - I swapped the parent node
  if(a->parent->right == b){ //change the right child to a
    a->parent->right = a;
  }else{
    a->parent->left = a; //change left child to a
  }
  if(b->parent->right == a){ //change the right child to b
    b->parent->right = b;
  }else{
    b->parent->left = b; //change the left child to b
  }
}
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO:
  postfix_traversal(ptr->left, nth, expression); //traverse left recursively
	postfix_traversal(ptr->right, nth, expression); //traverse right recursively
	if(ptr->module == NULL){ //node is cutline
		expression[* nth].cutline = ptr->cutline;//set cutline to pointer's cutline
    expression[* nth].module = NULL; //node is cutline, so set module to NULL using expression struct
	}else{ //node is module
    expression[* nth].module = ptr->module; //set module to a value using expression struct
    expression[* nth].cutline = UNDEFINED_CUTLINE; //node is module, so set cutline to undef
	}
  *nth += 1; //increment nth
}
...
int get_total_resource(node_t* ptr)
{
  // TODO:
  int sum = 0;
  if(ptr != NULL) { //would normally be a while loop, but since it is a recursive method, use if statement
    if(ptr->module != NULL) {	//if the module is not empty, set the sum to the resource of the module
      sum = ptr->module->resource;
    }
    sum = sum + get_total_resource(ptr->left) + get_total_resource(ptr->right); //implements a recursive call to traverse left and right
    return sum;
  }else{
    return 0; //means that the pointer is null
  } 
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // TODO:
  node_t * nodeC = (node_t *)malloc(sizeof(node_t)); //allocate memory
  //base case
  if (n == (num_modules - 1)){ //if the depth of the currrent recursion is == num of modules-1
    nodeC->module = modules + n; //set the nodeC to all parts of the node struct
    nodeC->cutline = UNDEFINED_CUTLINE; 
    nodeC->parent = par; //set nodeC to parent node as given in the header
    nodeC->left = NULL;
    nodeC->right = NULL;
		return nodeC;
  }
  ...
  //internal node
  nodeC->module = NULL;
  nodeC->cutline = V; //set cutline to V
  nodeC->parent = NULL;
  nodeC->left = NULL;
  nodeC->right = NULL;

  //right child
  nodeC->right = (node_t*)malloc(sizeof(node_t)); //allocate memory
  nodeC->right->module = modules + n; //set module to the modules + n
  nodeC->right->cutline = UNDEFINED_CUTLINE; 
  nodeC->right->parent = nodeC; //set the right child of nodeC to the nodeC
  nodeC->right->left = NULL;
  nodeC->right->right = NULL;

  //left child
  nodeC->left = init_slicing_tree(nodeC, n + 1); //recursively call the function to set the left child
  return nodeC;
}
